...
function insertMessage() {
  ...
  $('<div class="message message-personal">' + msg + '</div>').addClass('new')
  fetchmsg()
...
function serverMessage(response2) {
  ...
  setTimeout(function () {
    ...
    $('<div class="message new" id="qw">' + response2 + '</div>')
      .appendTo($('.mCSB_container'))
      .addClass('new')
    ...
    .then((response) => {
      ...
      try {
        document.getElementById('qw').innerHTML =
          ...
          '<br><img src="' +
          response.Reply.fulfillmentMessages[1].card.imageUri +
          '"/>' +
          '<div>' +
          response.Reply.fulfillmentMessages[1].card.title +
          '</div>' +
          '<div>' +
          response.Reply.fulfillmentMessages[2].text.text +
          '</div>'
      ...
      try {
        document.getElementById('qw').innerHTML =
          response.Reply.fulfillmentText +
          '<div>' +
          response.Reply.fulfillmentMessages[1].text.text +
          '</div>'
      ...
      try {
        document.getElementById('qw').innerHTML =
          '<div>' + response.Reply.fulfillmentMessages[1].text.text + '</div>'
        $('.carousel').carousel({
          interval: 3000,
        })
      } catch {}
      ...
      try {
        if (response.Reply.fulfillmentMessages[2].text.text == 'small') {
          ...
          $('.messages .messages-content').css('display', 'unset')
          ...
          $('.messages .messages-content').css('display', 'flex')
          $(document).ready(function () {
            $('.gif').animate({
              right: '-135px',
              top: '201px',
      ...
      try {
        ...
        var utterThis = new SpeechSynthesisUtterance(
          response.Reply.fulfillmentMessages[0].simpleResponses.simpleResponses[0].textToSpeech,
        )
        ...
        speechSynthesis.cancel()
        var utterThis = new SpeechSynthesisUtterance(
          response.Reply.fulfillmentMessages[0].text.text,
        )
        utterThis.voice = voices[2]
        utterThis.lang = 'hi-IN'
        synth.speak(utterThis)
